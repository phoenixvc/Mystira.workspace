...
pub async fn azure_preview_infrastructure(
    ...
    let preview_params_json = build_parameters_json(
        env,
        "southafricanorth",
        deploy_storage_val,
        deploy_cosmos_val,
    ...
    match preview_output {
        Ok(output) => {
            ...
            let has_invalid_response =
                stderr_str.contains("DeploymentWhatIfResourceInvalidResponse");
            let error_count_whatif = stderr_str.matches("DeploymentWhatIfResourceError").count();
            let error_count_invalid = stderr_str
                .matches("DeploymentWhatIfResourceInvalidResponse")
                .count();
            ...
            let has_valid_preview = parsed_json.is_some()
                && parsed_json
                    .as_ref()
