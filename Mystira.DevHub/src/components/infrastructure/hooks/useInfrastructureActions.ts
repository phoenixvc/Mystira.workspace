...
export function useInfrastructureActions({
  ...
  const handleAction = useCallback(
    async (action: "validate" | "preview" | "deploy" | "destroy") => {
      if (action !== "destroy") {
        ...
        if (selectedTemplates.length === 0) {
          onSetLastResponse({
            success: false,
            error:
              "Please select at least one template in Step 1 before proceeding.",
          });
          onSetLoading(false);
        ...
        if (deploymentMethod === "azure-cli") {
          if (!repoRoot || repoRoot.trim() === "") {
            onSetLastResponse({
              success: false,
              error:
                "Repository root not available. Please wait for it to be detected, or use GitHub Actions workflow instead.",
            });
            onSetLoading(false);
            onSetCurrentAction?.(null);
            return;
          ...
          switch (action) {
            ...
            case "preview": {
              ...
              if (response.success && response.result) {
                ...
                if (parsedChanges.length > 0) {
                  parsedChanges = parsedChanges.map((change) => ({
                    ...
                    resourceGroup:
                      ...
                      resourceGroupConfig.resourceTypeMappings?.[
                        change.resourceType
                      ] ||
                  ...
                  const errorStr =
                    response.error ||
                    (typeof previewData.errors === "string"
                      ? previewData.errors
                      : null) ||
                    (typeof previewData.errors === "object" &&
                    previewData.errors
                      ? JSON.stringify(previewData.errors)
                      : null) ||
                    "";
                  ...
                  if (hasCosmosWarning) {
                    onSetCosmosWarning({
                      type: "cosmos-whatif",
                      message:
                        "Cosmos DB nested resource errors detected during preview",
                      details:
                        errorStr ||
                        warningText ||
                        "Cosmos DB nested resource preview limitations",
                      affectedResources,
                      dismissed: false,
                    });
                  }
                ...
                const isOnlyCosmosErrors =
                  errorStr.includes("DeploymentWhatIfResourceError") &&
                  errorStr.includes("Microsoft.DocumentDB") &&
                  (errorStr.includes("sqlDatabases") ||
                    errorStr.includes("containers"));
            ...
            case "deploy": {
              ...
              if (!hasPreviewed) {
                onSetLastResponse({
                  success: false,
                  error:
                    "Please run Preview first to see what will be deployed before deploying.",
                });
                onSetLoading(false);
                onSetCurrentAction?.(null);
                return;
              ...
              if (selectedResources.length === 0) {
                onSetLastResponse({
                  success: false,
                  error:
                    "Please select at least one template in Step 1 to deploy.",
                });
                onSetLoading(false);
                onSetCurrentAction?.(null);
                return;
              ...
              if (selectedModules.has("appservice")) {
                if (
                  !selectedModules.has("cosmos") ||
                  !selectedModules.has("storage")
                ) {
                  onSetLastResponse({
                    success: false,
                    error:
                      "App Service requires Cosmos DB and Storage Account to be selected.",
                  });
                  onSetLoading(false);
                  onSetCurrentAction?.(null);
                  return;
            ...
            case "destroy": {
              response = {
                success: false,
                error:
                  "Destroy action not available for direct Azure CLI deployment.",
              };
              break;
            }
                ...
                onSetWhatIfChanges([
                  {
                    ...
                    changeType: "modify",
                    changes: [
                      "consistencyPolicy.defaultConsistencyLevel: BoundedStaleness → Session",
                    ],
                  },
                  {
  ...
  const handleDestroyConfirm = useCallback(async () => {
    ...
    try {
      ...
      onSetLastResponse({
        ...
        message: allSuccess
          ? `Successfully destroyed ${destroyResults.length} resource(s)`
          : undefined,
        error: allSuccess
          ? undefined
          : `Some resources failed to destroy:\n${errors}`,
      });
  ...
  const handleDeployConfirm = useCallback(
    async (onRefreshInfrastructureStatus?: () => void) => {
      onSetLoading(true);
      ...
      try {
        ...
        const resourcesByGroup = selectedResources.reduce(
          (acc, resource) => {
            const rg =
        ...
        for (const resourceGroup of resourceGroups) {
          ...
          if (
            !response.success &&
            response.result &&
            (response.result as any).needsCreation
          ) {
            // Resource group doesn't exist - prompt user for confirmation
            const location =
              (response.result as any).location ||
              resourceGroupConfig.region ||
              "southafricanorth";
          ...
          if (
            !response.success &&
            response.error &&
            (response.error.includes("ServiceUnavailable") ||
              response.error.includes("high demand"))
          ) {
            const regionMatch =
              response.error.match(/high demand in ([^,]+)/i) ||
              response.error.match(/region[^,]*([A-Z][a-z]+ [A-Z][a-z]+)/i);
            const region = regionMatch ? regionMatch[1] : "West Europe";
            const enhancedError =
              `❌ Region Capacity Issue: ${region} is currently experiencing high demand for Cosmos DB.\n\n` +
              `💡 Suggested Solutions:\n` +
            ...
            deploymentResults.push({
              ...
              logs:
                response.result && (response.result as any).logs
                  ? (response.result as any).logs
                  : undefined,
            });
          }
          // Check for Cosmos DB failed state error - extract actual account name from error
            ...
            !response.success &&
            response.error &&
            response.error.includes("failed provisioning state")
          ) {
            ...
            if (actualCosmosAccountName) {
              const errorMessage =
                `The Cosmos DB account "${actualCosmosAccountName}" is in a failed provisioning state from a previous deployment attempt. You need to delete it before recreating it.\n\n` +
                `To fix this:\n` +
        ...
        const errors = deploymentResults
          .filter((r) => !r.success)
          .map((r) => `${r.resourceGroup}: ${r.error}`)
          .join("\n");
        ...
        if (
          errors &&
          errors.includes("ServiceUnavailable") &&
          errors.includes("high demand")
        ) {
        ...
        const response: CommandResponse = {
          ...
          result: {
            ...
            logs: allLogs || undefined,
          },
          message: allSuccess
            ? `Successfully deployed to ${deploymentResults.length} resource group(s)`
            : undefined,
