...
export function useVSCodeLayout({
  ...
  const [layout, setLayout] = useState<LayoutState>(() => {
    ...
    return {
      ...DEFAULT_LAYOUT,
      activeBottomTab: defaultBottomTab || bottomPanelTabs[0]?.id || "",
    };
  });
  ...
  const startResize = useCallback(
    (type: "primary" | "secondary" | "bottom", e: React.MouseEvent) => {
      e.preventDefault();
  ...
  const togglePrimarySidebar = useCallback(() => {
    setLayout((prev) => ({
      ...prev,
      primarySidebarCollapsed: !prev.primarySidebarCollapsed,
    }));
  }, []);
  ...
  const toggleSecondarySidebar = useCallback(() => {
    setLayout((prev) => ({
      ...prev,
      secondarySidebarCollapsed: !prev.secondarySidebarCollapsed,
    }));
  }, []);
