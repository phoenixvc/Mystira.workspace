...
public class BadgeAdminService : IBadgeAdminService
{
        ...
        Dictionary<string, BadgeImage>? imageLookup)
    {
        var dto = new BadgeDto
        {
            ...
            CreatedAt = badge.CreatedAt ?? DateTime.UtcNow,
            UpdatedAt = badge.UpdatedAt ?? DateTime.UtcNow
        };
    ...
    private static string? ResolveSchemaPath()
    {
        ...
        var possiblePaths = new[]
        {
            ...
            Path.GetFullPath(Path.Combine(baseDirectory, "..", "..", "..", "..", "src", "Mystira.Domain", "Schemas", "badge-configuration.schema.json")),
            Path.GetFullPath(Path.Combine(baseDirectory, "..", "..", "Mystira.Domain", "Schemas", "badge-configuration.schema.json"))
        };
