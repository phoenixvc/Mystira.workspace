...
{
    var exitCode = await RunEventClassificationTestsAsync(host.Services, logger);
    return exitCode;
}
...
logger.LogInformation("  --test-entity-classifier   Runs example tests against the Entity Classifier using Ai settings from appsettings.json");
logger.LogInformation("  --test-event-classifier    Runs event-focused tests in parallel using Ai settings from appsettings.json");
return 0;

static async Task<int> RunEventClassificationTestsAsync(IServiceProvider services, ILogger logger)
{
