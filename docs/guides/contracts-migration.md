...
1. **Single Source of Truth**: All API types in one package per ecosystem
2. **Simplified Dependencies**: Add one package instead of multiple
3. **Consistent Versioning**: All contracts share the same version
4. **Better Discoverability**: All types in one searchable package
5. **Tree-Shaking Support**: Only import what you need via subpath exports
