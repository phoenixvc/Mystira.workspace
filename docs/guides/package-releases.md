...
This guide covers how to release packages in the Mystira workspace, including **NPM packages** and **Docker images**.

> **Note**: NuGet packages are no longer published. All .NET packages are consumed via `ProjectReference` within the monorepo (see [ADR-0024: True Monorepo Migration](../architecture/adr/0024-true-monorepo-migration.md)).
...
| Trigger           | NPM Packages           | Docker Images        |
| ----------------- | ---------------------- | -------------------- |
| Version PR merged | Published to npmjs.org | Tagged on next build |
...
### Checking .NET Packages

Since .NET packages are no longer published (consumed via `ProjectReference`):

```bash
# Build and verify packages locally
dotnet build packages/shared/Mystira.Shared
dotnet build packages/contracts/dotnet/Mystira.Contracts
````

> See [packages-publish.yml](../../.github/workflows/packages-publish.yml) for CI validation.
