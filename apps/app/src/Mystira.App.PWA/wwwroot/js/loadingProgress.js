...
window.mystiraLoading = (function () {
    ...
    if ("connection" in navigator) {
      ...
      if (connectionType === "slow-2g" || connectionType === "2g") {
        ...
        timeoutTimer = setTimeout(() => {
          console.warn(
            LOG_PREFIX,
            "Loading timeout reached (25s for slow connection)"
          );
  ...
  function handleServiceWorkerMessage(event) {
    ...
    switch (event.data?.type) {
      case "CACHE_PROGRESS":
        ...
        if (
          currentStageIndex <
          LOADING_STAGES.findIndex((s) => s.id === "downloading-content")
        ) {
  ...
  function updateStage(stageId) {
    ...
    const stageIndex = LOADING_STAGES.findIndex(
    ...
    const staticIcon = document.querySelector(
      "#static-progress-tracker .stage-icon i"
    );
    if (staticIcon) {
  ...
  function reportProblem() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard
        ...
        .then(() => {
          alert(
            "Diagnostic information has been copied to your clipboard. Please paste it when reporting the problem."
          );
        })
        .catch(() => {
  ...
  return {
    ...
    start: () => {
      ...
      if (currentStageIndex > 0) {
        console.log(
          LOG_PREFIX,
          "Loading already in progress (stage: " +
            LOADING_STAGES[currentStageIndex].id +
            "), not resetting"
        );
        return;
      }
      ...
      if (loadingInitialized) {
        ...
        console.warn(
          LOG_PREFIX,
          "Cannot start - not initialized yet, waiting for DOM..."
        );
...
if (document.readyState === "loading") {
  ...
  console.log(
    LOG_PREFIX,
    "DOM already loaded, waiting for manual initialization"
  );
  loadingInitialized = true;
}
