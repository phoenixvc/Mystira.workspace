    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === search.toLowerCase() ? (
          <mark
            key={i}
            className="bg-yellow-400 text-black dark:bg-yellow-600 dark:text-white"
          >
            {part}
          </mark>
        ) : (
...
export function isErrorMessage(message: string): boolean {
  ...
  if (
    msg.match(/^\d+\s+error\(s\)/i) ||
    msg.match(/^\d+\s+warning\(s\)/i) ||
    msg.match(/^\d+\s+errors/i) ||
    msg.match(/^\d+\s+warnings/i)
  ) {
    return false;
  }
  return (
