...
const variantStyles: Record<ButtonVariant, string> = {
  ...
  warning:
    "bg-yellow-500 text-black hover:bg-yellow-600 focus:ring-yellow-400 dark:bg-yellow-400 dark:hover:bg-yellow-500",
  ghost:
...
const actionCardVariants = {
  ...
  success:
    "border-green-200 dark:border-green-800 hover:bg-green-50 dark:hover:bg-green-900/30",
  warning:
    "border-yellow-200 dark:border-yellow-800 hover:bg-yellow-50 dark:hover:bg-yellow-900/30",
  danger:
...
export function ActionCard({
  ...
  return (
    ...
    >
      <span
        className={`${compact ? "text-base" : "text-lg"} ${loading ? "animate-spin" : ""}`}
      >
        {loading ? "⟳" : icon}
      </span>
      <span
        className={`${compact ? "text-[9px]" : "text-[10px]"} font-semibold mt-0.5`}
      >
        {label}
      </span>
      {description && !compact && (
