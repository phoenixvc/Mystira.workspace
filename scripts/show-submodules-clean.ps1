...
function Get-SubmoduleInfo {
    ...
    try {
        ...
        try {
            ...
            $upstreamRef = '@{u}'
            $remoteBranch = git rev-parse --abbrev-ref --symbolic-full-name $upstreamRef 2>$null
            $behind = 0
            $ahead = 0
            if ($remoteBranch -and $remoteBranch -ne 'HEAD') {
                git fetch --quiet 2>$null
                $behindStr = git rev-list --count "HEAD..$upstreamRef" 2>$null
                $aheadStr = git rev-list --count "$upstreamRef..HEAD" 2>$null
