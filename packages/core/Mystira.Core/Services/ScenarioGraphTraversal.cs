...
public static class ScenarioGraphTraversal
{
    ...
    {
        if (branch.CompassChange == null || branch.CompassChange.Axis == null ||
            string.IsNullOrWhiteSpace((string)branch.CompassChange.Axis))
            return;

        string axis = branch.CompassChange.Axis;
