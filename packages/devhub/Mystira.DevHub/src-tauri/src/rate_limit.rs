...
impl RateLimiter {
    ...
    pub async fn wait_if_needed(&self, service: &str, requests_per_minute: u32) {
        ...
        if let Some(wait_secs) = wait_seconds {
            debug!(
                "Rate limit reached for {}, waiting {} seconds",
                service, wait_secs
            );
...
pub async fn wait_azure_rate_limit() {
    let config = get_config();
    RATE_LIMITER
        .wait_if_needed("azure", config.rate_limit.azure_requests_per_minute)
        .await;
}
...
pub async fn wait_github_rate_limit() {
    let config = get_config();
    RATE_LIMITER
        .wait_if_needed("github", config.rate_limit.github_requests_per_minute)
        .await;
}
