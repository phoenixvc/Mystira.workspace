...
export const useSmartDeploymentStore = create<SmartDeploymentState>(
  (set, get) => ({
    ...
    startDeployment: async (repoRoot: string) => {
      ...
      for (let i = 0; i < regionPriority.length; i++) {
        ...
        try {
          ...
          const rgResponse = await invoke<CommandResponse>(
          ...
          const deployResponse = await invoke<CommandResponse>(
          ...
          if (deployResponse.success) {
            ...
            const deploymentResult: DeploymentResult = {
              ...
              region: regionId,
              resourceGroup,
              apiUrl: (result.outputs as Record<string, { value: string }>)
                ?.apiAppServiceUrl?.value,
              adminApiUrl: (result.outputs as Record<string, { value: string }>)
                ?.adminApiAppServiceUrl?.value,
              logs: result.logs as string,
            };

            addLog(`✓ Deployment successful in ${region.name}!`);
            addLog(`Resource Group: ${resourceGroup}`);
            if (deploymentResult.apiUrl)
              addLog(`API URL: ${deploymentResult.apiUrl}`);
            ...
            set((state) => {
              ...
              remaining.forEach((r) => {
                newStatuses.set(r, {
                  regionId: r,
                  status: "skipped",
                  message: "Not needed - deployment succeeded",
                });
              });
              return { regionStatuses: newStatuses };
            });
