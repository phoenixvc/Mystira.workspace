...
export const useDeploymentsStore = create<DeploymentsState>((set, get) => ({
  ...
  fetchDeployments: async (forceRefresh = false, limit?: number) => {
    ...
    try {
      ...
      const response = await invoke<CommandResponse<GitHubWorkflowRun[]>>(
      ...
      if (response.success && response.result) {
        const mappedDeployments: Deployment[] = response.result.map(
          (run, index) => {
            // Determine action type from workflow name
            let action: "deploy" | "validate" | "preview" | "destroy" =
              "deploy";
            if (run.name?.toLowerCase().includes("validate"))
              action = "validate";
            else if (
              run.name?.toLowerCase().includes("preview") ||
              run.name?.toLowerCase().includes("what-if")
            )
              action = "preview";
            else if (run.name?.toLowerCase().includes("destroy"))
              action = "destroy";
      ...
      if (
        ...
        set({
          isLoading: false,
          error:
            "Deployment history feature is not yet available. The backend command needs to be implemented.",
        });
      } else {
        set({
          isLoading: false,
