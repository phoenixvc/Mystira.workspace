...
export function useServiceOperations({
  ...
  const startService = async (serviceName: string) => {
    ...
    try {
      ...
      if (config?.port) {
        try {
          const available = await invoke<boolean>("check_port_available", {
            port: config.port,
          });
          if (!available) {
            onAddToast(
      ...
      if (
        errorMessage.includes("__TAURI_IPC__") ||
  ...
  const rebuildService = async (serviceName: string) => {
    ...
    try {
      const success = await prebuildService(
        serviceName,
        rootToUse,
  ...
  const startAllServices = async () => {
    ...
    if (servicesToStart.length === 0) {
      const buildingServices = serviceConfigs.filter(
