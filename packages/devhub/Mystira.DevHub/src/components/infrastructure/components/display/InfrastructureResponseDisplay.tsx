...
export function InfrastructureResponseDisplay({
      ...
      if (result.success) {
        ...
        if (onLastResponseChange) {
          onLastResponseChange({
            success: true,
            message:
              "Failed resource deleted successfully. You can now retry deployment.",
          });
        }
      } else {
        alert(
  ...
  const hasFailedProvisioningState =
    response?.error?.includes("failed provisioning state") || false;
  const failedResourceInfo = response?.error
    ? getFailedResourceInfo(response.error)
    : null;
            ...
            <div className="flex items-start gap-3 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-amber-700 dark:text-amber-300 mb-2">
                  Azure's what-if preview cannot predict changes for nested
                  Cosmos DB resources (databases and containers) that don't
                  exist yet. This is a known Azure limitation and does not
                  affect actual deployments.
                </p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-yellow-700 dark:text-yellow-300 mb-2">
                  These are informational warnings and do not prevent
                  deployment. You can safely proceed.
                </p>
                <details className="text-xs">
            ...
            {response.error && (
              <div className="mt-2">
                <p className="text-xs text-red-800 dark:text-red-200 mb-2">
                  Error details have been sent to the <strong>Problems</strong>{" "}
