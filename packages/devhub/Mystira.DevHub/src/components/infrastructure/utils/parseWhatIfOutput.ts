...
export function parseWhatIfOutput(whatIfJson: string): WhatIfChange[] {
  try {
    const parsed =
      typeof whatIfJson === "string" ? JSON.parse(whatIfJson) : whatIfJson;
    ...
    if (Array.isArray(parsed)) {
      ...
      parsed.resourceChanges &&
      Array.isArray(parsed.resourceChanges)
    ) {
      ...
      parsed.properties?.resourceChanges &&
      Array.isArray(parsed.properties.resourceChanges)
    ) {
    ...
    if (changesArray.length > 0) {
      changesArray.forEach((change: any) => {
        ...
        const resourceName =
          resourceIdParts[resourceIdParts.length - 1] || resourceId;

        let resourceType =
          change.resourceType ||
          change.targetResource?.type ||
          change.resource?.type ||
          (resourceIdParts.length >= 8
            ? `${resourceIdParts[6]}/${resourceIdParts[7]}`
            : "Unknown");
        ...
        let changeType: "create" | "modify" | "delete" | "noChange" =
          "noChange";
        const azChangeType = (change.changeType || change.action || "")
          .toLowerCase()
          .trim();
        ...
        if (
          ...
          azChangeType === "delete" ||
          azChangeType === "remove" ||
          azChangeType === "destroy"
        ) {
          changeType = "delete";
        } else if (
          azChangeType === "nochange" ||
          azChangeType === "ignore" ||
          azChangeType === "no-op"
        ) {
          changeType = "noChange";
