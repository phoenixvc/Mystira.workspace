...
describe("storageAccountUtils", () => {
  ...
  describe("extractStorageAccountName", () => {
    it('should extract from JSON format with "target" field', () => {
      const error =
        '{"code": "StorageAccountInAnotherResourceGroup", "target": "devsanappmystirastorage", "message": "The account is in another resource group"}';
      expect(extractStorageAccountName(error)).toBe("devsanappmystirastorage");
    });

    it("should extract from prose format", () => {
      const error =
        "The account devsanappmystirastorage is already in another resource group in this subscription.";
      expect(extractStorageAccountName(error)).toBe("devsanappmystirastorage");
    });

    it("should extract from target with different formatting", () => {
      const error =
        'Inner Errors: target: "mystorageacct123" message: conflict';
  ...
  describe("parseAzureDeleteError", () => {
    ...
    it("should return original error for unknown patterns", () => {
      const unknownError =
        "Some completely unknown error that we do not recognize";
