...
export function JsonViewer({
  ...
  if (data === null || data === undefined) {
    return (
      <div
        className={`text-gray-500 dark:text-gray-400 italic text-xs ${className}`}
      >
        null
      </div>
    );
  ...
  return (
    <div
      className={`rounded border border-gray-200 dark:border-gray-700 overflow-hidden ${className}`}
    >
      {/* Header */}
      ...
      {!isCollapsed && (
        ...
        >
          <code
            className={`block font-mono ${compact ? "text-[10px]" : "text-xs"} leading-relaxed`}
          >
            {highlightedElements}
          </code>
        </div>
