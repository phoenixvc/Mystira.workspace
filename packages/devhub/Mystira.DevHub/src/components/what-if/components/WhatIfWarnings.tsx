...
export function WhatIfWarnings({
  deleteCount,
  showSelection,
  localChanges,
  compact,
  ...
  const selectedModules = new Set(
    localChanges
      ...
      .map((c) => {
        ...
        if (
          type.includes("web/sites") ||
          type.includes("appservice") ||
          type.includes("web/serverfarms")
        )
          return "appservice";
        if (type.includes("documentdb") || type.includes("cosmos"))
          return "cosmos";
        if (type.includes("storage") && type.includes("account"))
          return "storage";
        return null;
      })
      .filter(Boolean),
      ...
      {deleteCount > 0 && (
        ...
        >
          <div className="flex items-start">
            <span className={compact ? "text-sm mr-2" : "text-2xl mr-3"}>
              ⚠️
            </span>
            <div>
              <div
                className={`font-semibold text-red-900 dark:text-red-300 ${compact ? "text-xs" : "mb-1"}`}
              >
                Warning: Destructive Changes
              </div>
              <div
                className={`${compact ? "text-[10px]" : "text-sm"} text-red-800 dark:text-red-400`}
              >
                {deleteCount} resource{deleteCount > 1 ? "s" : ""} will be
      ...
      {showSelection && missingDeps.length > 0 && (
        ...
        >
          <div className="flex items-start">
            ...
            <div>
              <div
                className={`font-semibold text-yellow-900 dark:text-yellow-300 ${compact ? "text-xs" : "mb-1"}`}
              >
                Dependency Warning
              </div>
              <div
                className={`${compact ? "text-[10px]" : "text-sm"} text-yellow-800 dark:text-yellow-400`}
              >
