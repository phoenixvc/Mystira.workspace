...
export function isValidSplits(splits: number[]): {
  valid: boolean;
  total: number;
  error?: string;
} {
  ...
  if (splits.some((split) => split > 100)) {
    return {
      valid: false,
      total,
      error: "Individual splits cannot exceed 100%",
    };
  }
...
export function hasMinContributors(
...
export function allApproved(approvalStatuses: string[]): boolean {
  return approvalStatuses.every(
    (status) => status === "approved" || status === "overridden"
  );
}
