...
export function ContributorForm({
  ...
  const { data: searchResults, isLoading: isSearching } = useQuery({
    ...
    queryFn: () =>
      contributorsApi.searchUsers({ query: searchQuery, limit: 5 }),
      ...
      <form onSubmit={handleSubmit} className="contributor-form">
        ...
        {useEmail ? (
          ...
          <div className="contributor-form__search">
            ...
            />
            {isSearching && (
              <span className="contributor-form__searching">Searching...</span>
            )}
              ...
              <ul className="contributor-form__results">
                {searchResults.map((user) => (
                  <li key={user.id}>
                    ...
                    >
                      <span>{user.name}</span>
                      <span className="contributor-form__result-email">
                        {user.email}
                      </span>
                    </button>
                  </li>
