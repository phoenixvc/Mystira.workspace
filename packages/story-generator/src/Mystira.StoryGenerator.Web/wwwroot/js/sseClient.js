...
window.sseClient = {
  ...
  connect: function (sessionId, url, dotNetRef) {
    ...
    if (this.eventSources[sessionId]) {
      console.log(
        `[SSE-JS] Closing existing connection for session ${sessionId} before reconnecting`
      );
    ...
    eventTypes.forEach((eventType) => {
      eventSource.addEventListener(eventType, (event) => {
        ...
        try {
          ...
          // Call back to .NET
          dotNetRef
            .invokeMethodAsync("OnSseEvent", eventType, event.data)
            .catch((err) =>
              console.error(`[SSE-JS] Error invoking .NET method:`, err)
            );
        } catch (err) {
    ...
    eventSource.onerror = (error) => {
      console.error(
        `[SSE-JS] Connection error for session ${sessionId}:`,
        error
      );
      ...
      if (dotNetRef) {
        dotNetRef
          .invokeMethodAsync("OnSseError", error.toString())
          .catch((err) =>
            console.error(`[SSE-JS] Error invoking .NET error handler:`, err)
          );
      }
