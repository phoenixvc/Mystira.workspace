  ...
  window.mystiraPanels = {
    init: function (config) {
      const container = document.getElementById(
        config.containerId || "panel-container"
      );
      const leftEl = document.getElementById(config.leftId || "panel-left");
      const centerEl = document.getElementById(
        config.centerId || "panel-center"
      );
      const rightEl = document.getElementById(config.rightId || "panel-right");
      const leftResizer = document.getElementById(
        config.leftResizerId || "resizer-left"
      );
      const rightResizer = document.getElementById(
        config.rightResizerId || "resizer-right"
      );
      if (
        !container ||
        !leftEl ||
      ...
      const mins = Object.assign(
        { left: 15, center: 25, right: 20 },
        config.mins || {}
      );
      const stored = load();
      ...
      function attachMove(type) {
        ...
        moveHandler = (ev) => {
          if (type === "left") onMouseMoveLeft(ev);
          else onMouseMoveRight(ev);
        };
        upHandler = () => {
