  ...
  if (!userEngagementTimer) {
    userEngagementTimer = setInterval(() => {
      ...
      if (engagementTime % 10000 === 0) {
        console.log(
          `PWA Install: User engagement time: ${engagementTime / 1000}s`
        );
      }

      // After sufficient engagement, try to trigger installation if possible (only once)
      if (
        engagementTime >= MIN_ENGAGEMENT_TIME &&
        !deferredPrompt &&
        !isAppInstalled() &&
        !hasCheckedCriteria
      ) {
        console.log(
          "PWA Install: Sufficient engagement reached, checking install criteria"
        );
...
function checkInstallCriteria() {
  ...
  if (deferredPrompt && deferredPrompt !== IOS_PROMPT) {
    console.log(
      "PWA Install: Deferred prompt available, updating button visibility"
    );
...
export function initializePwaInstall(dotNetReference) {
  ...
  if (!deferredPrompt && isIos() && !isAppInstalled()) {
    ...
    console.log(
      "PWA Install: Mobile device detected without native prompt, showing manual instructions"
    );
  ...
  if (navigator.userAgent.includes("Chrome") && !isIos() && !isAppInstalled()) {
    console.log(
      "PWA Install: Chrome detected, checking install criteria immediately"
    );
    setTimeout(() => {
      tryTriggerChromeInstall();
...
export async function installPwa() {
  ...
  if (!deferredPrompt) {
    console.log(
      "PWA Install: No deferred prompt available, trying to trigger install"
    );
  ...
  try {
    ...
    if (choiceResult.outcome === "accepted") {
      console.log(
        "PWA Install: User accepted installation - marking as installed"
      );
      localStorage.setItem(INSTALL_FLAG_KEY, "true");
    } else {
