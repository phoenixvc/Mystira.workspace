      ...
      try {
        ...
        for (const url of iconUrls) {
          try {
            ...
            console.log(
              `Service Worker: Cached icon ${iconCompleted}/${iconUrls.length}: ${url}`
            );
          } catch (error) {
        ...
        for (const url of coreUrls) {
          try {
            ...
            notifyProgress(
              "cache",
              iconCompleted + coreCompleted,
              iconUrls.length + coreUrls.length
            );
            console.log(
              `Service Worker: Cached core file ${coreCompleted}/${coreUrls.length}: ${url}`
            );
          } catch (error) {
            console.warn(
              `Service Worker: Failed to cache core file: ${url}`,
              error
            );
            coreCompleted++;
            progressData.coreProgress.current = coreCompleted;
        ...
        const totalCompleted =
          progressData.iconProgress.current + progressData.coreProgress.current;
      ...
      await caches.keys().then((cacheNames) => {
        return Promise.all(
          cacheNames.map((cache) => {
            ...
            if (
              cache !== ICON_CACHE_NAME &&
              cache !== CORE_CACHE_NAME &&
              cache !== "mystira-version-cache"
            ) {
...
async function clearAllCaches() {
  ...
  try {
    ...
    const cachesToClear = cacheKeys.filter(
      (key) => key !== "mystira-version-cache"
    );
...
async function checkVersionAndInvalidate() {
  try {
    ...
    console.log(
      `${LOG_PREFIX} Version check - stored: ${storedVersion}, new: ${newVersion}`
    );

    if (storedVersion && storedVersion !== newVersion) {
      console.log(
        `${LOG_PREFIX} Version changed from ${storedVersion} to ${newVersion} - clearing caches`
      );
